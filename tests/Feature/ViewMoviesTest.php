...
{
    public function testMainPageShowsCorrectInfo (): void
    {
        Http::fake([
        ...
        $response->assertSeeTextInOrder(['Adventure', 'Drama', 'Mystery', 'Science Fiction', 'Thriller']);
    ...
    public function testMoviePageShowsCorrectInfo(): void
    {
        Http::fake([
            "https://api.themoviedb.org/3/movie/*" => $this->fakeSingleMovie()
        ]);

        $response = $this->get(route('movies.show', Random::generate(4, 0-9)));

        $response->assertSeeText('Featured Crew');
        $response->assertSeeText('Play Trailer');
        $response->assertSeeText('Fake Jumanji: The Next Level');
        $response->assertSeeText('Dr. Smolder Bravestone');
        $response->assertSee('rBxcF-r9Ibs');
        $response->assertSee('/bB42KDdfWkOvmzmYkmK58ZlCa9P.jpg');
        $response->assertSeeTextInOrder(['Adventure', 'Drama', 'Mystery', 'Science Fiction', 'Thriller']);


    }

    public function testSearchDropdownWorksCorrectly(): void
    {
        Http::fake([
